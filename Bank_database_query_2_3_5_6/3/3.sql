...
-- Start Query
-- find customer who has account but not loan
Select depositor.customerName from depositor
left join borrower on depositor.customerName=borrower.customerName
where depositor.customerName not in (select customerName from borrower);

select avg(balance) as avarageBalance from account
where branchName='Mohakhali';